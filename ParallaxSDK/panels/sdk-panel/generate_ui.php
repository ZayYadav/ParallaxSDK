...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $package_mode = $package_lock ? 'SPECIFIC' : 'ANY';
    ...
    if (!in_array($signing_mode, ['pinned', 'auto', 'any'], true)) {
        $_SESSION['gen_error'] = 'Choose a valid APK signing policy.';
        header('Location: generate_ui.php');
        exit();
    }
    if ($signing_mode === 'pinned' && preg_match('/^[A-F0-9]{64}$/D', $signing_cert) !== 1) {
        $_SESSION['gen_error'] = 'Exact signing mode requires a 64-character SHA-256 certificate fingerprint.';
        header('Location: generate_ui.php');
        exit();
    }
    $signing_lock = $signing_mode === 'any' ? 0 : 1;
    $signing_cert_value = $signing_mode === 'pinned' ? $signing_cert : null;
    $signing_mode_db = $signing_mode === 'pinned' ? 'SPECIFIC' : strtoupper($signing_mode);
    $license_status = isset($_POST['license_status']) && (string) $_POST['license_status'] === '0' ? 0 : 1;
    ...
    $featuresInput = trim((string) ($_POST['feature_policy'] ?? ''));
    $featurePolicy = $featuresInput === '' ? [] : json_decode($featuresInput, true);
    $blockedVersions = array_values(array_unique(array_filter(array_map(
        'intval',
        preg_split('/[\s,]+/', trim((string) ($_POST['blocked_versions'] ?? ''))) ?: []
    ), static fn(int $version): bool => $version > 0 && $version <= 1000000)));

    if ($client_name === '' || mb_strlen($client_name) > 120) {
        $_SESSION['gen_error'] = 'Client / App Name is required (maximum 120 characters).';
        header('Location: generate_ui.php');
        exit();
    }
    if (!in_array($device_mode, ['DISABLED', 'SINGLE', 'LIMITED', 'UNLIMITED'], true)) {
        $_SESSION['gen_error'] = 'Choose a valid device binding policy.';
        header('Location: generate_ui.php');
        exit();
    }
    if (!is_array($featurePolicy)) {
        $_SESSION['gen_error'] = 'Feature Policy must be a valid JSON object.';
        header('Location: generate_ui.php');
        exit();
    }
    foreach ($featurePolicy as $feature => $value) {
        if (!is_string($feature) || preg_match('/^[A-Za-z0-9_.-]{1,64}$/D', $feature) !== 1
            ...
            $_SESSION['gen_error'] = 'Feature Policy supports simple named boolean, number or string values only.';
            header('Location: generate_ui.php');
            exit();
        }
    }
    ...
    if ($expiryDate && $expiryDate->format('Y-m-d') === $expiry && $expiryDate >= new DateTimeImmutable('today', new DateTimeZone('UTC'))) {
        ...
        $stmt = mysqli_prepare(
            ...
            "INSERT INTO licenses
                (license_key, client_name, expiry_date, status, package_name, package_lock,
                 package_mode, signing_lock, signing_mode, signing_cert_sha256,
                 device_mode, max_devices, java_native_auth, feature_policy,
                 minimum_sdk_version, latest_sdk_version, force_update, blocked_versions,
                 session_lifetime_seconds, kill_switch, generated_by)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
        );
        ...
        $_SESSION['success_license'] = [
            ...
            'signing_cert' => $signing_cert_value,
            'key_type'     => $key_type,
            'status'       => $license_status === 1 ? 'ACTIVE' : 'INACTIVE'
        ];
...
<div class="main">
    ...
    <div class="glass form-card">
        ...
        <form method="POST" id="licenseForm">
            ...
            <div class="mb-3">
                <label><i class="fas fa-building me-2" style="color:var(--p-primary);"></i>Client / App Name</label>
                <input name="client_name" class="form-control" maxlength="120" required
                       placeholder="e.g., Client A — Production App">
            </div>
            ...
            <div class="mb-3">
                <label><i class="fas fa-certificate me-2" style="color:var(--p-primary);"></i>APK Signing Policy</label>
                <select name="signing_mode" id="signingMode" class="form-select" onchange="onSigningModeChange(this.value)" required>
                    <option value="pinned">Exact signing SHA-256 — strongest</option>
                    <option value="auto" selected>Auto-bind first valid activation</option>
                    <option value="any">Any signing key — compatibility mode</option>
                </select>
                <small id="signingModeHelp" style="color:rgba(255,255,255,.48);font-size:.72rem;margin-top:5px;display:block;line-height:1.5;">
                    The first successful v3 activation permanently binds this license to that APK certificate.
                </small>
            </div>

            <div class="mb-3" id="signingCertWrap" style="display:none;">
                <label><i class="fas fa-fingerprint me-2" style="color:var(--p-primary);"></i>Signing Certificate SHA-256</label>
                <input name="signing_cert_sha256" id="signingCertInput" class="form-control"
                       maxlength="95" autocomplete="off"
                       placeholder="AA:BB:CC:... or 64 hex characters">
                <small style="color:rgba(255,255,255,.4);font-size:.72rem;margin-top:4px;display:block;">
                    Paste the APK signing certificate fingerprint; colons and spaces are accepted.
                </small>
            </div>

            <div id="anySigningWarning" style="display:none;background:rgba(245,158,11,.12);border:1px solid rgba(245,158,11,.35);border-radius:12px;padding:12px 14px;margin-bottom:16px;color:#fde68a;font-size:.78rem;line-height:1.5;">
                <i class="fas fa-triangle-exclamation me-2"></i>
                Any signing key disables repackaged-APK protection for this license. Panel signatures, device proof, package lock and short leases remain enabled.
            </div>
            ...
            <div class="mb-3">
                <label><i class="fas fa-shield-halved me-2" style="color:var(--p-primary);"></i>Device Binding</label>
                <select name="device_mode" id="deviceMode" class="form-select" onchange="onDeviceModeChange(this.value)" required>
                    <option value="DISABLED">Disabled</option>
                    <option value="SINGLE" selected>Single Device</option>
                    <option value="LIMITED">Limited Devices</option>
                    <option value="UNLIMITED">Unlimited Devices</option>
                </select>
            </div>

            <div class="mb-3" id="maxDevicesWrap">
                <label><i class="fas fa-mobile-alt me-2" style="color:var(--p-primary);"></i>Maximum Devices</label>
                <input type="number" name="max_devices" id="maxDevices" class="form-control" min="1" max="100" value="1" required>
            ...
            <div class="mb-3">
                <label><i class="fas fa-sliders me-2" style="color:var(--p-primary);"></i>Feature Policy (JSON)</label>
                <textarea name="feature_policy" class="form-control" rows="3"
                          placeholder='{"feature_a":true,"feature_b":false}'></textarea>
            </div>

            <div class="row g-3 mb-3">
                <div class="col-md-6">
                    <label>Minimum SDK Version</label>
                    <input type="number" name="minimum_sdk_version" class="form-control" min="1" value="3" required>
                </div>
                <div class="col-md-6">
                    <label>Latest SDK Version</label>
                    <input type="number" name="latest_sdk_version" class="form-control" min="1" value="3" required>
                </div>
            </div>

            <div class="mb-3">
                <label>Blocked SDK Versions</label>
                <input name="blocked_versions" class="form-control" placeholder="e.g., 1, 2, 7">
            </div>

            <div class="mb-3">
                <label>Authorization Session Lifetime</label>
                <select name="session_lifetime_seconds" class="form-select">
            ...
            <div class="form-check mb-2">
                <input class="form-check-input" type="checkbox" name="java_native_auth" value="1" id="javaNativeAuth" checked>
                <label class="form-check-label" for="javaNativeAuth">Require Java + Native cross-authentication</label>
            </div>
            <div class="form-check mb-2">
                <input class="form-check-input" type="checkbox" name="force_update" value="1" id="forceUpdate">
                <label class="form-check-label" for="forceUpdate">Force update below latest version</label>
            </div>
            <div class="form-check mb-3">
                <input class="form-check-input" type="checkbox" name="kill_switch" value="1" id="killSwitch">
                <label class="form-check-label" for="killSwitch">Create with kill switch enabled</label>
            </div>

            <div class="mb-3">
                <label>Initial License Status</label>
...
<div class="success-overlay active" id="successModal">
    <div class="success-card glass">
        ...
        <?php endif; ?>
        <div class="success-row"><span>APK Signing</span><strong>
            <?php if (($s['signing_mode'] ?? 'auto') === 'pinned'): ?>
                Exact: <?= htmlspecialchars(substr((string) $s['signing_cert'], 0, 12)) ?>…
            <?php elseif (($s['signing_mode'] ?? 'auto') === 'any'): ?>
                Any signing key
            <?php else: ?>
                Auto-bind first activation
            <?php endif; ?>
        </strong></div>
...
function onSigningModeChange(mode) {
    const certWrap = document.getElementById('signingCertWrap');
    const certInput = document.getElementById('signingCertInput');
    const warning = document.getElementById('anySigningWarning');
    const help = document.getElementById('signingModeHelp');
    ...
    help.textContent = mode === 'pinned'
        ? 'Only an APK signed by this exact certificate can activate the license.'
        : mode === 'any'
            ? 'All APK signing certificates are accepted for this license.'
            : 'The first successful v3 activation permanently binds this license to that APK certificate.';
}

function onDeviceModeChange(mode) {
    const wrap = document.getElementById('maxDevicesWrap');
    const input = document.getElementById('maxDevices');
    const limited = mode === 'LIMITED';
    wrap.style.display = (mode === 'SINGLE' || limited) ? 'block' : 'none';
    input.required = mode === 'SINGLE' || limited;
    if (mode === 'SINGLE') input.value = '1';
}
...
window.onload = function() {
    ...
    onSigningModeChange(document.getElementById('signingMode').value);
    onDeviceModeChange(document.getElementById('deviceMode').value);
};
</script>
