...
function sdk_feature_telegram_login_hmac_key(): string
{
    $secret = trim((string)(function_exists('panel_config') ? panel_config('PANEL_DATA_KEY', '') : ''));
    if (strlen($secret) < 32) {
        throw new RuntimeException('Telegram 2FA server key is not configured.');
    }
...
function sdk_feature_login_intercept(mysqli $conn): void
{
    ...
    // Existing TOTP/recovery-code MFA remains authoritative. Telegram 2FA is
    // never allowed to replace or bypass it.
    ...
    if ($chatId === '') {
        sdk_feature_render_login_challenge('Telegram 2FA is enabled but no linked Telegram account is available. Contact the owner for recovery.', false);
    }
    ...
    try {
        $key = sdk_feature_telegram_login_hmac_key();
    } catch (Throwable $e) {
        error_log('SDK panel Telegram 2FA configuration error: ' . $e->getMessage());
        sdk_feature_render_login_challenge('Telegram verification is temporarily unavailable. Contact the owner.', false);
    }
    ...
    $ins->bind_param('is', $uid, $hash);
    if (!$ins->execute()) {
        $ins->close();
        sdk_feature_render_login_challenge('Could not create a Telegram verification challenge.', false);
    }
    $challengeId = (int)$conn->insert_id;
    $ins->close();

    session_regenerate_id(true);
    $_SESSION['sdk_pending_telegram_2fa'] = [
        'user_id' => $uid,
    ...
    $delivered = sdk_feature_telegram_send(
        $chatId,
        '<b>Parallax SDK login code</b>\n<code>' . $code . '</code>\nExpires in 5 minutes. Do not share this code.'
    );
    if (!$delivered) {
        $consume = $conn->prepare('UPDATE telegram_login_challenges SET consumed_at=UTC_TIMESTAMP() WHERE id=?');
        $consume->bind_param('i', $challengeId);
        $consume->execute();
        $consume->close();
        unset($_SESSION['sdk_pending_telegram_2fa']);
        sdk_feature_audit($conn, 'telegram_2fa_challenge', 'delivery_failed', $uid, $uid);
        sdk_feature_render_login_challenge('Telegram could not deliver the login code. Try again later or contact the owner.', false);
    }
...
function sdk_feature_render_login_challenge(string $message, bool $showForm): never
{
    ...
    if ($showForm) {
        echo '<form method="post">' . $csrf . '<input name="telegram_code" inputmode="numeric" pattern="[0-9]{8}" maxlength="8" placeholder="00000000" required autofocus><button name="verify_telegram_2fa" value="1">Verify & sign in</button></form>';
    }
...
function sdk_feature_verify_telegram_login(mysqli $conn): never
{
    ...
    $stmt = $conn->prepare('SELECT id,username,status,auth_version,telegram_2fa_enabled,telegram_chat_id FROM users WHERE id=? LIMIT 1');
    $stmt->bind_param('i', $uid);
    $stmt->execute();
    $user = $stmt->get_result()->fetch_assoc();
    $stmt->close();
    if (!$user || (int)$user['status'] !== 1 || (int)$user['telegram_2fa_enabled'] !== 1
        || empty($user['telegram_chat_id']) || (int)$user['auth_version'] !== (int)$pending['auth_version']) {
        unset($_SESSION['sdk_pending_telegram_2fa']);
    ...
    $q->bind_param('ii', $challengeId, $uid);
    $q->execute();
    ...
    try {
        ...
        unset($_SESSION['sdk_pending_telegram_2fa']);
        error_log('SDK panel Telegram 2FA configuration error: ' . $e->getMessage());
        sdk_feature_render_login_challenge('Telegram verification is temporarily unavailable.', false);
    }
    $expected = hash_hmac('sha256', $code, $key);
    if (!hash_equals((string)$row['code_hash'], $expected)) {
        $inc = $conn->prepare('UPDATE telegram_login_challenges SET attempts=attempts+1 WHERE id=? AND consumed_at IS NULL');
        $inc->bind_param('i', $challengeId);
        $inc->execute();
        $inc->close();
        sdk_feature_audit($conn, 'telegram_2fa_verify', 'failed', $uid, $uid);
        if ((int)$row['attempts'] + 1 >= 5) {
            unset($_SESSION['sdk_pending_telegram_2fa']);
            sdk_feature_render_login_challenge('Too many incorrect codes. Sign in again to request a new challenge.', false);
        }
        sdk_feature_render_login_challenge('Invalid code. Try again; the current challenge is still active.', true);
    }

    $consume = $conn->prepare('UPDATE telegram_login_challenges SET consumed_at=UTC_TIMESTAMP() WHERE id=? AND consumed_at IS NULL');
    $consume->bind_param('i', $challengeId);
    $consume->execute();
    $ok = $consume->affected_rows === 1;
    $consume->close();
    if (!$ok) {
        unset($_SESSION['sdk_pending_telegram_2fa']);
...
function sdk_feature_process_broadcast_batch(mysqli $conn, int $broadcastId, int $limit = 25): array
{
    ...
    $b->bind_param('i', $broadcastId);
    $b->execute();
    $broadcast = $b->get_result()->fetch_assoc();
    $b->close();
    if (!$broadcast) {
        return ['sent' => 0, 'failed' => 0, 'remaining' => 0];
    }

    // A worker/browser can disappear after claiming a recipient. Recover stale
    // claims so they can retry rather than remaining permanently stuck.
    $recover = $conn->prepare(
        "UPDATE announcement_recipients SET status='failed',claimed_at=NULL,last_error='Recovered stale delivery claim' "
        . "WHERE broadcast_id=? AND status='sending' AND (claimed_at IS NULL OR claimed_at<UTC_TIMESTAMP()-INTERVAL 5 MINUTE)"
    );
    $recover->bind_param('i', $broadcastId);
    $recover->execute();
    $recover->close();

    $markBroadcast = $conn->prepare("UPDATE announcement_broadcasts SET status='sending',completed_at=NULL WHERE id=? AND status IN ('queued','sending','complete')");
    $markBroadcast->bind_param('i', $broadcastId);
    $markBroadcast->execute();
    $markBroadcast->close();
    ...
    if ($res) {
        while ($r = $res->fetch_assoc()) {
            ...
            // Atomic claim prevents two owner requests from sending the same
            // recipient concurrently.
            $claim = $conn->prepare(
                "UPDATE announcement_recipients SET status='sending',attempts=attempts+1,claimed_at=UTC_TIMESTAMP() "
                . "WHERE id=? AND broadcast_id=? AND status IN ('queued','failed') AND attempts<3"
            );
            $claim->bind_param('ii', $rid, $broadcastId);
            $claim->execute();
            ...
            $ok = sdk_feature_telegram_send(
                $chat,
                '<b>Official announcement</b>\n' . htmlspecialchars((string)$broadcast['message'], ENT_QUOTES, 'UTF-8')
            );
            if ($ok) {
                $done = $conn->prepare("UPDATE announcement_recipients SET status='sent',sent_at=UTC_TIMESTAMP(),claimed_at=NULL,last_error='' WHERE id=? AND status='sending'");
                $done->bind_param('i', $rid);
                $done->execute();
                $done->close();
                $sent++;
            } else {
                $err = 'Telegram delivery failed';
                $done = $conn->prepare("UPDATE announcement_recipients SET status='failed',claimed_at=NULL,last_error=? WHERE id=? AND status='sending'");
                $done->bind_param('si', $err, $rid);
                $done->execute();
                $done->close();
                $failedThisBatch++;
            }
    ...
    $countsResult = $conn->query(
        ...
        . "COALESCE(SUM(status='failed' AND attempts>=3),0) failed,"
        . "COALESCE(SUM(status='queued' OR status='sending' OR (status='failed' AND attempts<3)),0) remaining "
        . 'FROM announcement_recipients WHERE broadcast_id=' . (int)$broadcastId
    );
    $counts = $countsResult ? ($countsResult->fetch_assoc() ?: []) : [];
    $totalSent = (int)($counts['sent'] ?? 0);
    $terminalFailed = (int)($counts['failed'] ?? 0);
    $remaining = (int)($counts['remaining'] ?? 0);
    $status = $remaining === 0 ? 'complete' : 'sending';

    $stmt = $conn->prepare(
        "UPDATE announcement_broadcasts SET status=?,sent_count=?,failed_count=?,"
        . "completed_at=CASE WHEN ?='complete' THEN UTC_TIMESTAMP() ELSE NULL END WHERE id=?"
    );
    $stmt->bind_param('siisi', $status, $totalSent, $terminalFailed, $status, $broadcastId);
    $stmt->execute();
    $stmt->close();

    return ['sent' => $sent, 'failed' => $failedThisBatch, 'remaining' => $remaining];
}
...
function sdk_feature_queue_broadcast(mysqli $conn, int $actorId, string $message, bool $panel, bool $linked, bool $guest): int
{
    ...
    if ($message === '' || mb_strlen($message) > 1000) {
        throw new RuntimeException('Announcement must be 1-1000 characters.');
    }
    if (!$panel && !$linked && !$guest) {
    ...
    try {
        ...
        if ($panel) {
            ...
            $a->bind_param('sssii', $title, $message, $type, $active, $actorId);
            if (!$a->execute()) {
                throw new RuntimeException('Could not create panel announcement.');
            }
            $announcementId = (int)$conn->insert_id;
            $a->close();
            if (!sdk_feature_save_setting($conn, 'site_announcement', $message, $actorId)) {
                throw new RuntimeException('Could not publish site announcement.');
            }
        }
        $pi = $panel ? 1 : 0;
        $li = $linked ? 1 : 0;
        ...
        $b->bind_param('isiiii', $announcementId, $message, $pi, $li, $gi, $actorId);
        if (!$b->execute()) {
            throw new RuntimeException('Could not create broadcast queue.');
        }
        ...
        if ($linked || $guest) {
            ...
            $res = $conn->query('SELECT id,chat_id FROM telegram_users WHERE ' . implode(' OR ', $where));
            ...
            while ($res && ($row = $res->fetch_assoc())) {
                ...
                $r->bind_param('iis', $broadcastId, $tgId, $chat);
                if (!$r->execute()) {
                    throw new RuntimeException('Could not snapshot Telegram recipients.');
                }
            }
            $r->close();
        }
        $conn->commit();
        sdk_feature_audit($conn, 'announcement_publish', 'success', $actorId, null, [
