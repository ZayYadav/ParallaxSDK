...
| X / Twitter | Legacy Twitter Kit app-to-app login is used only when the installed provider genuinely exports `SingleSignOnActivity`. Current X builds that removed that component are not given a fabricated SSO successor. Real Twitter/X OAuth URLs are handed to the installed X URL interpreter when Android confirms it can handle them, and the official AccountAuthenticator remains available through the system account bridge. | OAuth1 request-token/browser flows still require the exact callback URL approved for the application's consumer key; the SDK cannot override provider-side callback policy. New integrations should use OAuth 2.0 Authorization Code with PKCE and `state`. |
