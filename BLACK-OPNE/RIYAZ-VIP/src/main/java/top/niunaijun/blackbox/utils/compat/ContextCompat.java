...
public class ContextCompat {
    ...
    /**
     * Repairs the identity that Android system services observe for a virtual
     * Context. The virtual package still lives in BlackBox's own package manager;
     * calls crossing into the real framework run under the host UID and therefore
     * must carry the host package as well.
     *
     * Each hidden-field update is deliberately isolated. On Android 16 and OEM
     * builds a single missing/renamed hidden field must not skip every subsequent
     * identity repair (the previous all-or-nothing try/catch could do exactly
     * that and leave GMS/AppOps seeing a virtual package paired with the host UID).
     */
    ...
    private static Context unwrap(Context context) {
        ...
        try {
            ...
            // Keep the deepest context obtained so far.
        }
        return current;
    }
}
