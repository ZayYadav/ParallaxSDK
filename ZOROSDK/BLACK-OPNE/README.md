# ZOROSDKCore Android SDK

ZOROSDKCore is the reusable Android library consumed by the ZOROSDK Loader application.
...
ZOROSDKCore routes supported authentication surfaces to the authoritative app
...
Android package/signing identity; ZOROSDKCore does not forge signatures, defeat
...
```bash
./gradlew :ZOROSDKCore:assembleRelease
```

The release AAR is written to
`RIYAZ-VIP/build/outputs/aar/ZOROSDKCore-release.aar`.
...
```bash
export ZOROSDK_AAR_ENCRYPTION_KEY="<base64-32-byte-client-key>"
./gradlew :ZOROSDKCore:encryptReleaseAar
```

The authenticated AES-256-GCM envelope is written to
`RIYAZ-VIP/build/outputs/encrypted-aar/ZOROSDKCore-v3-release.aar.pxa`.
Before Gradle can consume it, the authorized client must decrypt it locally:

```bash
export ZOROSDK_AAR_ENCRYPTION_KEY="<same-client-key>"
./gradlew :ZOROSDKCore:decryptReleaseAar \
  -PzorosdkEncryptedAar=/secure/path/ZOROSDKCore-v3-release.aar.pxa
```
