...
Android loader application for the TeamDarkCore SDK.
...
TeamDark Loader intentionally produces a single 64-bit ARM APK. Its
...
The primary downloaded artifact is named `TeamDark.so`. Downloads must use
...
AAR first, copy it to `app/libs/TeamDarkCore-release.aar`, and then run:
...
- Login is pinned to `https://teamdarkloader.teamdarkserver.online/api/v2/connect` and the `PUBG`
...
- Release CI and direct release tasks require valid
  `TeamDark_API_PUBLIC_KEY_B64` and `TeamDark_TLS_PINS` configuration. No shared
