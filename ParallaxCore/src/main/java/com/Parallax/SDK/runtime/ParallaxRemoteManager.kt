...
@Obfuscate
class ParallaxRemoteManager private constructor() : IParallaxRemoteManager.Stub() {

    companion object {
    
         @JvmField
         val JUNIT_JAR = File(ParallaxEnvironment.getCacheDir(), "junit.apk")
         
         @JvmField
         val EMPTY_JAR = File(ParallaxEnvironment.getCacheDir(), "empty.apk")
        ...
        @Volatile
        private var instance: ParallaxRemoteManager? = null
        ...
        @JvmStatic
        fun getInstance(): ParallaxRemoteManager {
            return instance ?: synchronized(this) {
                instance ?: ParallaxRemoteManager().also { instance = it }
            }
        }
    }
    ...
    override fun activateSdk(userkey: String?) {
        ...
        if (normalizedKey.isEmpty()) {
            ParallaxSecurityState.clearActivation("Activation key is required")
            return
        }
        exe.execute {
            val context = ParallaxCore.getContext()
            val packageName = ParallaxCore.getHostPkg().orEmpty()
            if (packageName.isEmpty()) {
                ParallaxSecurityState.clearActivation("Host package is unavailable")
                return@execute
            }
            val client = ParallaxSecureSdkApiClient(context)
            ...
            for (attempt in 0..MAX_RETRIES) {
                try {
                    ...
                    ParallaxSecurityState.clearActivation(lastFailure)
                    if (attempt < MAX_RETRIES && isRetryable(throwable)) {
                        ParallaxSecurityState.Msg = "Secure connection retry ${attempt + 1}/$MAX_RETRIES"
                        try {
    ...
    private fun applySecureResponse(context: Context, data: JSONObject, licenseKey: String) {
        ...
        if (data.optString("status") != "success" || serverMode != "online") {
            ParallaxSecurityState.clearActivation(message)
        ...
        if (leaseExpiresAt <= serverTime || leaseExpiresAt <= System.currentTimeMillis() / 1000L) {
            ParallaxSecurityState.clearActivation("Invalid activation lease")
            return
        }
        val packageName = context.packageName
        val signingSha256 = ParallaxSecureSdkApiClient(context).appSigningCertificateSha256(packageName)
        val nativeAuthorized = ParallaxNative.authorizeSdkSession(
            packageName,
            signingSha256,
        ...
        if (data.optInt("java_native_auth", 1) == 1 && !nativeAuthorized) {
            ParallaxSecurityState.clearActivation("Native authorization cross-check failed")
            return
        }
        val expiry = data.optString("expiry", "")
        context.getSharedPreferences(ParallaxSecurityState.PREFERENCE_NAME, Context.MODE_PRIVATE).edit()
            ...
            .apply()
        ParallaxActivationBackup.save(context, licenseKey)
        ParallaxSecurityState.setHidden("online")
        ParallaxSecurityState.Msg = "SDK activated - secure lease verified"
        renewalTask?.cancel(false)
    ...
    fun restoreActivationFromBackupIfNeeded(reason: String = "startup") {
        ...
        exe.execute {
            try {
                val context = ParallaxCore.getContext() ?: return@execute
                val prefs = context.getSharedPreferences(ParallaxSecurityState.PREFERENCE_NAME, Context.MODE_PRIVATE)
                if (prefs.getBoolean("activated", false) && ParallaxSecurityState.getActivatedSdk()) {
                    return@execute
                }
                val savedKey = ParallaxActivationBackup.restore(context)
                if (savedKey.isNullOrBlank()) {
                    ParallaxSecurityState.Msg = "SDK not activated"
                    return@execute
                }
                ParallaxSecurityState.Msg = "Restoring SDK activation"
                activateSdk(savedKey)
            } catch (throwable: Throwable) {
                ParallaxSecurityState.Msg = "SDK auto restore failed"
            }
        }
    }
    ...
    /* Legacy v1 plaintext activation is intentionally excluded from bytecode.
        val bc: String
        try {
            val nc = Class.forName("com.Parallax.SDK.runtime.ParallaxSecurityState")
            val m = nc.getMethod("getUrlHidden")
            bc = m.invoke(null) as String
        } catch (e: Exception) {
            e.printStackTrace()
            ParallaxSecurityState.Msg = "Error: Failed to get API URL - ${e.message}"
            return
        }

        if (!iv(bc)) {
            ParallaxSecurityState.Msg = "Error: Invalid API URL format"
            return
        }

        if (userkey == null || userkey.trim().isEmpty()) {
            ParallaxSecurityState.Msg = "Error: User key cannot be empty"
            return
        }
        ...
        exe.execute {
            ...
            while (retryCount <= MAX_RETRIES && !success) {
                ...
                try {
                    val ctx = ParallaxCore.getContext()
                    val pkg = ParallaxCore.getHostPkg() ?: ""

                    if (pkg.isEmpty()) {
                        ParallaxSecurityState.Msg = "Error: Package name not found"
                        success = true
                        return@execute
                    }
                    ...
                    if (rc != HttpURLConnection.HTTP_OK) {
                        ...
                        ParallaxSecurityState.setHidden("Offline")
                        ParallaxSecurityState.Msg = lastError
                        ...
                        if (retryCount < MAX_RETRIES) {
                            retryCount++
                            ParallaxSecurityState.Msg = "Server Error: Retrying... ($retryCount/${MAX_RETRIES})"
                            Thread.sleep(3000)
                            continue
                        } else {
                            showNotificationSafe("SDK ACTIVATE FAILED", "SERVER ERROR: $rc")
                    ...
                    try {
                        ...
                        if (retryCount < MAX_RETRIES) {
                            retryCount++
                            ParallaxSecurityState.Msg = "Read Timeout: Retrying... ($retryCount/${MAX_RETRIES})"
                            Thread.sleep(3000)
                            continue
                        } else {
                            throw ste
                    ...
                    if (res.isEmpty()) {
                        ...
                        if (retryCount < MAX_RETRIES) {
                            retryCount++
                            ParallaxSecurityState.Msg = "Empty Response: Retrying... ($retryCount/${MAX_RETRIES})"
                            Thread.sleep(3000)
                            continue
                        } else {
                            ParallaxSecurityState.setHidden("Offline")
                            ParallaxSecurityState.Msg = "Error: Empty response from server"
                    ...
                    if (serverMode.equals("maintenance", true)) {
                        // MAINTENANCE MODE
                        ParallaxSecurityState.setHidden("Offline")
                        ParallaxSecurityState.Msg = "SERVER MAINTENANCE: ${if (serverMessage.isNotEmpty()) serverMessage else "Server under maintenance"}"
                    ...
                    if (serverMode.equals("offline", true)) {
                        // OFFLINE MODE
                        ParallaxSecurityState.setHidden("Offline")
                        ParallaxSecurityState.Msg = "SERVER OFFLINE: ${if (serverMessage.isNotEmpty()) serverMessage else "Server is offline"}"
                    ...
                    if (data.getString("status") == "success") {
                        ParallaxSecurityState.setHidden("online")
                        ParallaxSecurityState.Msg = "✅ Sdk Activated Successfully"
                        ...
                        val sp = ParallaxCore.getContext().getSharedPreferences(ParallaxSecurityState.PREFERENCE_NAME, Context.MODE_PRIVATE)
                        ...
                        if (exp.isNotEmpty()) {
                            try {
                                ...
                                if (expiryDate != null) {
                                    ...
                                    if (currentTime < expiryTime) {
                                        ...
                                        ParallaxSecurityState.Msg = "✅ Licence Valid (${remainingDays} days remaining)"
                                        
                                    } else {
                                        // ❌ Licence already expired
                                        ParallaxSecurityState.Msg = "❌ Licence Expired on $exp"
                                ...
                                ParallaxSecurityState.Msg = "Licence saved (date parse error)"
                            }
                        } else {
                            // No expiry date
                            sp.edit().apply {
                            ...
                            ParallaxSecurityState.Msg = "✅ Licence Activated (No expiry)"
                        }
                        ...
                        ParallaxSecurityState.Msg = "❌ ACTIVATE Failed: $reason"
                    ...
                    if (retryCount < MAX_RETRIES) {
                        retryCount++
                        ParallaxSecurityState.Msg = "Timeout: Retrying... ($retryCount/${MAX_RETRIES})"
                        Thread.sleep(3000)
                        continue
                    } else {
                        ParallaxSecurityState.setHidden("Offline")
                        ParallaxSecurityState.Msg = "Error: Connection timeout"
                    ...
                    if (retryCount < MAX_RETRIES) {
                        retryCount++
                        ParallaxSecurityState.Msg = "Connect Error: Retrying... ($retryCount/${MAX_RETRIES})"
                        Thread.sleep(3000)
                        continue
                    } else {
                        ParallaxSecurityState.setHidden("Offline")
                        ParallaxSecurityState.Msg = "Error: Cannot connect to server"
                    ...
                    ParallaxSecurityState.setHidden("Offline")
                    ParallaxSecurityState.Msg = "Error: Invalid server URL"
                    ...
                    if (retryCount < MAX_RETRIES) {
                        retryCount++
                        ParallaxSecurityState.Msg = "Error: Retrying... ($retryCount/${MAX_RETRIES})"
                        Thread.sleep(3000)
                        continue
                    } else {
                        ParallaxSecurityState.setHidden("Offline")
                        ParallaxSecurityState.Msg = "Unexpected Error: ${e.message}"
            ...
            if (!success) {
                ...
                ParallaxSecurityState.Msg = "Failed after $MAX_RETRIES attempts: $finalError"
    ...
    */
    ...
    override fun getActivatedSdk(): Boolean {
        return try {
            val result = ParallaxSecurityState.getActivatedSdk()
            ParallaxSecurityState.Msg = if (result) "✅ SDK IS ACTIVATED" else "❌ SDK IS NOT ACTIVATED"
            result
        } catch (e: Exception) {
            ParallaxSecurityState.Msg = "ERROR: FAILED TO GET ACTIVATE STATUS"
            false
        }
    }
    ...
    override fun getServerMessage(): String {
        return try {
            val msg = ParallaxSecurityState.getServerMessage()
    ...
    override fun getNetwork(): Boolean {
        return try {
            val net = ParallaxSecurityState.isSystemApp()
            ParallaxSecurityState.Msg = if (net) "✅ Network: Connected" else "❌ Network: Disconnected"
            net
        } catch (e: Exception) {
            ParallaxSecurityState.Msg = "Error: Failed to check network status"
            false
        }
    }
    ...
    private fun isDaemon(d: Boolean) {
        if (d) {
            ParallaxSecurityState.Msg = "Daemon: ENABLED"
            sEnableDaemonService = true
        } else {
            ParallaxSecurityState.Msg = "Daemon: DISABLED"
            sEnableDaemonService = false
        }
    }
    ...
    private fun ishideRoot(h: Boolean) {
        if (h) {
            ParallaxSecurityState.Msg = "Root Hide: ENABLED"
            sHideRoot = true
        } else {
            ParallaxSecurityState.Msg = "Root Hide: DISABLED"
            sHideRoot = false
        }
    }
