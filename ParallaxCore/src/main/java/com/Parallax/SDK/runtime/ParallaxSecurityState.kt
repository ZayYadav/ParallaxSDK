...
class ParallaxSecurityState {
    ...
    companion object {
        ...
        fun getActivatedSdk(): Boolean {
            val context = ParallaxCore.getContext() ?: return false
            ...
            if (effectiveNow >= leaseExpiry || !ParallaxNative.isSdkSessionValid(effectiveNow)) {
        ...
        fun clearActivation(reason: String = "SDK not activated") {
            is_False = false
            try {
                ParallaxNative.clearSdkSession()
            } catch (_: Throwable) {
            }
            try {
                ParallaxCore.getContext()?.getSharedPreferences(PREFERENCE_NAME, Context.MODE_PRIVATE)
                    ?.edit()
        ...
        fun ismsg(msg: String?) {
            if (msg == null) return
            val ctx = ParallaxCore.getContext() ?: return
        ...
        @JvmStatic
        fun 获取接口地址(): String {
            return ParallaxNative.getSdkPanelEndpoint()
        }
        ...
        fun isSystemApp(): Boolean {
            ...
            if (!GAH()) {
                ...
                try {
                    ParallaxAdvancedPopupHelper.showAuto()
                } catch (_: Exception) {}
                return false
            }
        ...
        fun checkExpiryManually(): String {
            val context = ParallaxCore.getContext() ?: return "No context"
            val sp = context.getSharedPreferences(PREFERENCE_NAME, Context.MODE_PRIVATE)
        ...
        fun loadSavedStatus() {
            try {
                ...
                if (!getActivatedSdk()) {
                    is_False = false
                    try {
                        ParallaxRemoteManager.getInstance().restoreActivationFromBackupIfNeeded("loadSavedStatus")
                    } catch (_: Throwable) {}
                }
            } catch (_: Exception) {}
        }
