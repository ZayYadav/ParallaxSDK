 ...
 */
public class ParallaxUIDSpoofingHelper {
    private static final String TAG = "ParallaxUIDSpoofingHelper";
    ...
    public static int getSystemUID() {
        try {
            ...
            ParallaxSlog.w(TAG, "Failed to get system UID, using fallback", e);
    ...
    public static int getPackageUID(String packageName) {
        try {
            Context context = ParallaxCore.getContext();
            if (context != null) {
                ...
                if (pm != null) {
                    ...
                    if (uid > 0) { // Valid UIDs are positive numbers
                        ParallaxSlog.d(TAG, "Found real UID for package " + packageName + ": " + uid);
                        return uid;
                    }
                }
            }
            ...
            ParallaxSlog.w(TAG, "Package UID not found for " + packageName + ", using system UID");
            return getSystemUID();
            
        } catch (Exception e) {
            ParallaxSlog.w(TAG, "Failed to get package UID for " + packageName + ", using system UID", e);
            return getSystemUID();
        }
    }
    ...
    public static int getJobSchedulingUID(String targetPackage) {
        try {
            ...
            if (packageUid > 0) { // Valid UIDs are positive numbers
                ParallaxSlog.d(TAG, "Using package UID for job scheduling: " + packageUid);
                return packageUid;
            }
            
            // Fallback to system UID
            ParallaxSlog.w(TAG, "Using system UID for job scheduling as fallback");
            return getSystemUID();
            
        } catch (Exception e) {
            ParallaxSlog.w(TAG, "Failed to get job scheduling UID, using system UID", e);
            return getSystemUID();
        }
    }
    ...
    public static boolean needsUIDSpoofing(String operation, String targetPackage) {
        try {
            ...
            ParallaxSlog.d(TAG, "UID spoofing check: current=" + currentUid + 
                       ", target=" + targetUid + ", needsSpoofing=" + needsSpoofing);
            
            return needsSpoofing;
            
        } catch (Exception e) {
            ParallaxSlog.w(TAG, "Failed to check UID spoofing need, assuming yes", e);
    ...
    public static int getBestUIDForOperation(String operation, String targetPackage) {
        try {
            ...
            ParallaxSlog.w(TAG, "Failed to get best UID for operation " + operation + ", using system UID", e);
            return getSystemUID();
        }
    }
    ...
    public static void logUIDInfo(String operation, String targetPackage) {
        try {
            ...
            ParallaxSlog.d(TAG, String.format("UID Info - Operation: %s, Package: %s, Current: %d, Best: %d, NeedsSpoofing: %s",
                    operation, targetPackage, currentUid, bestUid, needsSpoofing));
                    
        } catch (Exception e) {
            ParallaxSlog.w(TAG, "Failed to log UID info", e);
        }
    }
}
