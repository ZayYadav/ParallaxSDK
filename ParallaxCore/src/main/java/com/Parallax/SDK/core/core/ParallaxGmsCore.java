...
import com.Parallax.SDK.core.utils.auth.ParallaxAuth;

public class ParallaxGmsCore {
    ...
    private static ApplicationInfo applyAnalyticsSafety(ApplicationInfo info) {
        ...
        if (info.packageName.equals(ParallaxCore.getHostPkg())
    ...
    public static boolean setGoogleAppOrService(String pkg) {
        ...
        for (String p : ParallaxAuth.AUTH_PKG_SET) {
    ...
    private static ParallaxInstallResult installPackages(Set<String> list, int userId) {
        ParallaxCore sBlackBoxCore = ParallaxCore.get();
        for (String packageName : list) {
            ...
            try {
                ParallaxCore.getContext().getPackageManager().getApplicationInfo(packageName, 0);
            } catch (PackageManager.NameNotFoundException ignored) {
                continue;
            }

            ParallaxInstallResult installResult = sBlackBoxCore.installPackageAsUser(packageName, userId);
            if (!installResult.success) {
                return installResult;
            }
        }
        return new ParallaxInstallResult();
    }
    ...
    public static ParallaxInstallResult installGApps(int userId) {
        Set<String> googleApps = new HashSet<>();
        googleApps.addAll(GOOGLE_SERVICE);
        googleApps.addAll(GOOGLE_APP);

        ParallaxInstallResult installResult = installPackages(googleApps, userId);
        if (!installResult.success) {
    ...
    public static boolean isSupportGms() {
        try {
            ParallaxCore.getPackageManager().getPackageInfo(GMS_PKG, 0);
            return true;
        } catch (PackageManager.NameNotFoundException ignored) {
            return false;
        }
    ...
    public static boolean isInstalledGoogleService(int userId) {
        return ParallaxCore.get().isInstalled(GMS_PKG, userId);
    }
}
