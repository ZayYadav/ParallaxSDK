 ...
 */
final class ParallaxDeclaredOAuthCallbackResolver {
    ...
    private ParallaxDeclaredOAuthCallbackResolver() {
    }
    ...
    static List<Uri> findCandidates(String virtualPackage, int userId) {
        ...
        try {
            ApplicationInfo appInfo = ParallaxPackageManager.get().getApplicationInfo(
    ...
    private static List<Uri> parseCandidates(File apk, String virtualPackage) {
        ...
        try {
            PackageParser parser = ParallaxPackageParserCompat.createParser(apk);
            if (parser == null) {
                return Collections.emptyList();
            }
            PackageParser.Package parsed = ParallaxPackageParserCompat.parsePackage(parser, apk, 0);
