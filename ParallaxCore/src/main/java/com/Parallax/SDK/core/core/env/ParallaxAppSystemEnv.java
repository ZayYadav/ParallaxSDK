...
public class ParallaxAppSystemEnv {
    ...
    public static boolean isOpenPackage(Intent intent) {
        ...
        if (ParallaxGmsCore.isMeasurementIntent(intent)) {
            return false;
        }
    ...
    public static boolean isBlackPackage(String packageName) {
        if (ParallaxCore.get().setHideRoot() && sSuPackages.contains(packageName)) {
            return true;
        }
        return ParallaxRemoteManager.sHideXposed && sXposedPackages.contains(packageName);
    }
