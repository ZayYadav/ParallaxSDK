 ...
 */
@Obfuscate
public final class ParallaxVirtualOAuthBridgeActivity extends Activity {
    private static final String TAG = "ParallaxCoreOAuth";
    private static final String AUTH_TAG = "ParallaxCoreAuth";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        externalAuthMode = launchIntent != null
                && launchIntent.getBooleanExtra(ParallaxExternalAuthRouter.EXTRA_EXTERNAL_AUTH, false);
        resultBridgeMode = launchIntent != null
                && launchIntent.getBooleanExtra(ParallaxExternalAuthRouter.EXTRA_BROWSER_AUTH, false);
        manualResultRelay = launchIntent != null
                && launchIntent.getBooleanExtra(
                ParallaxExternalAuthRouter.EXTRA_MANUAL_RESULT_RELAY, false);
        resultBpid = launchIntent == null ? -1
                : launchIntent.getIntExtra(ParallaxExternalAuthRouter.EXTRA_BPID, -1);

        if (externalAuthMode) {
            virtualPackage = launchIntent.getStringExtra(
                    ParallaxExternalAuthRouter.EXTRA_VIRTUAL_PACKAGE);
            userId = launchIntent.getIntExtra(ParallaxExternalAuthRouter.EXTRA_USER_ID, -1);
        ...
        String authUrl = launchIntent == null ? null
                : launchIntent.getStringExtra(ParallaxVirtualOAuthRouter.EXTRA_AUTH_URL);
        String redirectUriValue = launchIntent == null ? null
                : launchIntent.getStringExtra(ParallaxVirtualOAuthRouter.EXTRA_REDIRECT_URI);
        virtualPackage = launchIntent == null ? null
                : launchIntent.getStringExtra(ParallaxVirtualOAuthRouter.EXTRA_VIRTUAL_PACKAGE);
        userId = launchIntent == null ? -1
                : launchIntent.getIntExtra(ParallaxVirtualOAuthRouter.EXTRA_USER_ID, -1);
        authProvider = launchIntent == null ? null
                : launchIntent.getStringExtra(ParallaxVirtualOAuthRouter.EXTRA_AUTH_PROVIDER);
        ...
        if (!redirectResolvesToVirtualPackage(redirectUri)
                || !ParallaxAuthTabCompat.isSupportedProvider(this, authProvider, authUri)) {
        ...
        if (savedInstanceState == null) {
            if (facebookFlow) {
                ParallaxFacebookOAuthSessionStore.begin(
    ...
    private void launchExternalProvider(Intent bridgeIntent) {
        try {
            ...
            String providerPackage = ParallaxExternalAuthRouter.getTrustedProviderPackage(providerIntent);
    ...
    private void launchAuthTab(Uri authUri, String redirectScheme, String provider) {
        try {
            ...
            authIntent.putExtra(ParallaxAuthTabCompat.EXTRA_REDIRECT_SCHEME, redirectScheme);
            ...
            if (facebookFlow) {
                ParallaxFacebookOAuthSessionStore.clear(virtualPackage, userId);
            }
            if (resultBridgeMode) {
                completeBridgeResult(RESULT_CANCELED, null);
            } else {
    ...
    private void handleFacebookAuthResult(int resultCode, Intent data) {
        ...
        if (callbackUri != null && matchesExpectedCallback(callbackUri)) {
            ParallaxFacebookOAuthSessionStore.Claim claim =
                    ParallaxFacebookOAuthSessionStore.claim(callbackUri);
            boolean alreadyDelivered = ParallaxFacebookOAuthSessionStore.isCompleted(
            ...
            if (!delivered && claim != null
                ...
                if (delivered) {
                    ParallaxFacebookOAuthSessionStore.complete(claim.generation);
                } else {
                    ParallaxFacebookOAuthSessionStore.release(claim.generation);
                }
            }
        ...
        mainHandler.postDelayed(() -> {
            ...
            boolean delivered = ParallaxFacebookOAuthSessionStore.isCompleted(
                    virtualPackage, userId);
            facebookDiagnostic(
    ...
    private boolean validResultTarget(Intent launchIntent) {
        ...
        String targetPackage = extras.getString(ParallaxExternalAuthRouter.EXTRA_VIRTUAL_PACKAGE);
    ...
    private boolean relayOriginalActivityResult(int resultCode, Intent data) {
        try {
            ...
            ParallaxBundleCompat.putBinder(
                    relay, ParallaxExternalAuthRouter.EXTRA_RESULT_BINDER, resultTo);
            relay.putString(ParallaxExternalAuthRouter.EXTRA_RESULT_WHO, resultWho);
            relay.putInt(ParallaxExternalAuthRouter.EXTRA_REQUEST_CODE, originalRequestCode);
            relay.putInt(ParallaxExternalAuthRouter.EXTRA_RESULT_CODE, resultCode);
            relay.putInt(ParallaxExternalAuthRouter.EXTRA_BPID, resultBpid);
            relay.putInt(ParallaxExternalAuthRouter.EXTRA_USER_ID, userId);
            relay.putString(ParallaxExternalAuthRouter.EXTRA_VIRTUAL_PACKAGE, targetPackage);
            if (data != null) {
                relay.putParcelable(
                        ParallaxExternalAuthRouter.EXTRA_RESULT_DATA, new Intent(data));
            }

            Bundle response = ParallaxProviderCall.callSafely(
                    ParallaxProxyManifest.getProxyAuthorities(resultBpid),
                    ParallaxExternalAuthRouter.METHOD_DELIVER_ACTIVITY_RESULT,
                    null,
                    relay);
            return response != null
                    && response.getBoolean(
                    ParallaxExternalAuthRouter.EXTRA_RESULT_DELIVERED, false);
        } catch (Throwable ignored) {
            return false;
        }
    }
    ...
    private String providerPackageFromBridge() {
        try {
            ...
            if (providerIntent != null) {
                return ParallaxExternalAuthRouter.getTrustedProviderPackage(providerIntent);
            }
    ...
    private boolean matchesExpectedCallback(Uri callbackUri) {
        return ParallaxOAuthCallbackValidator.matches(authUri, expectedRedirectUri, callbackUri)
    ...
    private boolean dispatchToVirtualPackage(Uri callbackUri) {
        ...
        try {
            ...
            ResolveInfo resolved = ParallaxPackageManager.get().resolveActivity(
                    callback,
            ...
            ParallaxActivityManager.get().startActivity(callback, userId);
            return true;
        } catch (Throwable ignored) {
            return false;
        }
        ...
        if (virtualActivityExists(
            ...
            try {
                ...
                callback.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP
                        | Intent.FLAG_ACTIVITY_SINGLE_TOP);
                ParallaxActivityManager.get().startActivity(callback, targetUserId);
    ...
    private static boolean virtualActivityExists(
        ...
        try {
            ActivityInfo info = ParallaxPackageManager.get().getActivityInfo(
    ...
    private void facebookDiagnostic(String stage,
        ...
        boolean validated = uriPresent
                && ParallaxOAuthCallbackValidator.matches(authUri, expectedRedirectUri, callbackUri);
        boolean virtualTarget = validated && redirectResolvesToVirtualPackage(callbackUri);
        boolean fallbackCompleted = ParallaxFacebookOAuthSessionStore.isCompleted(
                virtualPackage, userId);
    ...
    private static void authDiagnostic(String stage, String provider, boolean delivered) {
        String safeProvider = ParallaxExternalAuthRouter.isTrustedProviderPackage(provider)
                ? provider : "unknown";
