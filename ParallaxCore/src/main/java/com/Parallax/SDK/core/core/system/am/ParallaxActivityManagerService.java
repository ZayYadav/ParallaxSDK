 ...
 * 此处无Bug
 */
public class ParallaxActivityManagerService extends IParallaxActivityManagerService.Stub implements IParallaxSystemService {
    public static final String TAG = "ParallaxActivityManagerService";
    private static final ParallaxActivityManagerService sService = new ParallaxActivityManagerService();
    private final Map<Integer, ParallaxUserSpace> mUserSpace = new HashMap<>();
    private final ParallaxPackageManagerService mPms = ParallaxPackageManagerService.get();
    private final ParallaxBroadcastManager mBroadcastManager;

    public static ParallaxActivityManagerService get() {
        return sService;
    }

    public ParallaxActivityManagerService() {
        mBroadcastManager = ParallaxBroadcastManager.startSystem(this, mPms);
    }

    @Override
    public ComponentName startService(Intent intent, String resolvedType, boolean requireForeground, int userId) {
        ParallaxUserSpace userSpace = getOrCreateSpaceLocked(userId);
    ...
    public IBinder acquireContentProviderClient(ProviderInfo providerInfo) throws RemoteException {
        ...
        ParallaxProcessRecord processRecord = ParallaxProcessManagerService.get().startProcessLocked(providerInfo.packageName,providerInfo.processName,ParallaxProcessManagerService.get().getUserIdByCallingPid(callingPid),-1,Binder.getCallingPid());
    ...
    public Intent sendBroadcast(Intent intent, String resolvedType, int userId) throws RemoteException {
        List<ResolveInfo> resolves = ParallaxPackageManagerService.get().queryBroadcastReceivers(intent, GET_META_DATA, resolvedType, userId);

        for (ResolveInfo resolve : resolves) {
            ParallaxProcessRecord processRecord = ParallaxProcessManagerService.get().findProcessRecord(resolve.activityInfo.packageName, resolve.activityInfo.processName, userId);
            if (processRecord == null) {
                continue;
            }
            try {
        ...
        shadow.setPackage(ParallaxCore.getHostPkg());
    ...
    public void onActivityCreated(int taskId, IBinder token, IBinder activityRecord) throws RemoteException {
        int callingPid = Binder.getCallingPid();
        ParallaxProcessRecord process = ParallaxProcessManagerService.get().findProcessByPid(callingPid);
        if (process == null) {
            return;
        }
        ParallaxActivityRecord record = (ParallaxActivityRecord) activityRecord;
        ParallaxUserSpace userSpace = getOrCreateSpaceLocked(process.userId);
    ...
    public ParallaxRunningAppProcessInfo getRunningAppProcesses(String callerPackage, int userId) throws RemoteException {
        ...
        List<ParallaxProcessRecord> packageProcessAsUser = ParallaxProcessManagerService.get().getPackageProcessAsUser(callerPackage, userId);

        ParallaxRunningAppProcessInfo appProcessInfo = new ParallaxRunningAppProcessInfo();
        for (ParallaxProcessRecord processRecord : packageProcessAsUser) {
    ...
    public void scheduleBroadcastReceiver(Intent intent, ParallaxPendingResultData pendingResultData, int userId) throws RemoteException {
        List<ResolveInfo> resolves = ParallaxPackageManagerService.get().queryBroadcastReceivers(intent, GET_META_DATA, null, userId);

        if (resolves.isEmpty()) {
            pendingResultData.build().finish();
            ParallaxSlog.d(TAG, "scheduleBroadcastReceiver empty");
            return;
        }
        ...
        for (ResolveInfo resolve : resolves) {
            ParallaxProcessRecord processRecord = ParallaxProcessManagerService.get().findProcessRecord(resolve.activityInfo.packageName, resolve.activityInfo.processName, userId);
            if (processRecord != null) {
                ParallaxReceiverData data = new ParallaxReceiverData();
                data.intent = intent;
    ...
    @Override
    public void finishBroadcast(ParallaxPendingResultData data) throws RemoteException {
    ...
    public void getIntentSender(IBinder target, String packageName, int uid, int userId) {
        ...
        synchronized (userSpace.mIntentSenderRecords) {
            ParallaxPendingIntentRecord record = new ParallaxPendingIntentRecord();
            record.uid = uid;
    ...
    public String getPackageForIntentSender(IBinder target, int userId) throws RemoteException {
        ParallaxUserSpace userSpace = getOrCreateSpaceLocked(userId);
        synchronized (userSpace.mIntentSenderRecords) {
            ParallaxPendingIntentRecord record = userSpace.mIntentSenderRecords.get(target);
            if (record != null) {
	...
	public int checkPermission(String permission, int pid, int uid, String packageName) throws RemoteException {
        ...
        return ParallaxPackageManagerService.get().checkUidPermission(permission, uid,packageName);
    }
    ...
    @Override
    public ParallaxUnbindRecord onServiceUnbind(Intent proxyIntent, int userId) throws RemoteException {
        ParallaxUserSpace userSpace = getOrCreateSpaceLocked(userId);
        synchronized (userSpace.mActiveServices) {
    ...
    @Override
    public ParallaxAppConfig initProcess(String packageName, String processName, int userId) throws RemoteException {
        ParallaxProcessRecord processRecord = ParallaxProcessManagerService.get().startProcessLocked(packageName, processName, userId, -1, Binder.getCallingPid());
        if (processRecord == null)
            return null;
    ...
    public void restartProcess(String packageName, String processName, int userId) throws RemoteException {
        ParallaxProcessManagerService.get().restartAppProcess(packageName, processName, userId);
    }
    ...
    public int startActivityAms(int userId, Intent intent, String resolvedType, IBinder resultTo, String resultWho, int requestCode, int flags, Bundle options) throws RemoteException {
        ParallaxUserSpace space = getOrCreateSpaceLocked(userId);
    ...
    private ParallaxUserSpace getOrCreateSpaceLocked(int userId) {
        synchronized (mUserSpace) {
            ParallaxUserSpace userSpace = mUserSpace.get(userId);
            if (userSpace != null) return userSpace;
            userSpace = new ParallaxUserSpace();
