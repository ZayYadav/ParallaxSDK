package com.Parallax.SDK.core.compat.auth;
...
import com.Parallax.SDK.core.compat.oauth.ParallaxVirtualOAuthBridgeActivity;
...
public final class ParallaxExternalAuthRouter {
    ...
    private ParallaxExternalAuthRouter() {
    }
    ...
    public static Intent createResultBridgeIntent(
        ...
        int bpid = ParallaxActivityThread.getAppPid();
        if (bpid < 0 || bpid > 24) {
            return null;
        }
    ...
    private static Intent createBaseBridge(
        ...
        bridge.setComponent(new ComponentName(
                ParallaxCore.getHostPkg(),
                ParallaxVirtualOAuthBridgeActivity.class.getName()));
        bridge.putExtra(EXTRA_EXTERNAL_AUTH, true);
        bridge.putExtra(EXTRA_BPID, bpid);
        bridge.putExtra(EXTRA_USER_ID, ParallaxActivityThread.getUserId());
    ...
    private static String resolveNativeTwitterProvider(Intent source) {
        ...
        try {
            PackageManager packageManager = ParallaxCore.getContext().getPackageManager();
    ...
    private static Intent createTwitterWebResultBridgeIntent(
            ...
            String virtualPackage) {
        try {
            Intent template = ParallaxVirtualOAuthRouter.createBridgeIntent(
                    source, ParallaxActivityThread.getUserId(), virtualPackage);
            if (template == null) {
                return null;
            }
            ...
            String authUrl = template.getStringExtra(ParallaxVirtualOAuthRouter.EXTRA_AUTH_URL);
            String redirectUri = template.getStringExtra(ParallaxVirtualOAuthRouter.EXTRA_REDIRECT_URI);
            String authProvider = template.getStringExtra(ParallaxVirtualOAuthRouter.EXTRA_AUTH_PROVIDER);
            int userId = template.getIntExtra(ParallaxVirtualOAuthRouter.EXTRA_USER_ID, -1);
            ...
            bridge.putExtra(ParallaxVirtualOAuthRouter.EXTRA_AUTH_URL, authUrl);
            bridge.putExtra(ParallaxVirtualOAuthRouter.EXTRA_REDIRECT_URI, redirectUri);
            bridge.putExtra(ParallaxVirtualOAuthRouter.EXTRA_AUTH_PROVIDER, authProvider);
            bridge.putExtra(ParallaxVirtualOAuthRouter.EXTRA_VIRTUAL_PACKAGE, virtualPackage);
            bridge.putExtra(ParallaxVirtualOAuthRouter.EXTRA_USER_ID, userId);
            putResultTarget(bridge, resultTo, resultWho, requestCode, virtualPackage);
            bridge.addFlags(source.getFlags() & (
                    Intent.FLAG_ACTIVITY_NO_ANIMATION
                            | Intent.FLAG_ACTIVITY_CLEAR_TOP
    ...
    private static Intent prepareBridgeForLaunch(Intent bridge) {
        ParallaxIntentRedirectCompat.collectNestedIntentKeys(bridge);
        return bridge;
    }
