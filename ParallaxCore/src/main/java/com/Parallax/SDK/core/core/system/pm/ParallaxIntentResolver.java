 ...
 * {@hide}
 */
public abstract class ParallaxIntentResolver<F extends ParallaxPackage.IntentInfo, R extends Object> {
    final private static String TAG = "ParallaxIntentResolver";
    ...
    public void addFilter(F f) {
        if (localLOGV) {
            ParallaxSlog.v(TAG, "Adding filter: " + f);
            f.intentFilter.dump(new LogPrinter(Log.VERBOSE, TAG), "      ");
            ParallaxSlog.v(TAG, "    Building Lookup Maps:");
        }
    ...
    void removeFilterInternal(F f) {
        if (localLOGV) {
            ParallaxSlog.v(TAG, "Removing filter: " + f);
            f.intentFilter.dump(new LogPrinter(Log.VERBOSE, TAG), "      ");
            ParallaxSlog.v(TAG, "    Cleaning Lookup Maps:");
        }
    ...
    public List<R> queryIntentFromList(Intent intent, String resolvedType, boolean defaultOnly,
        ...
        ParallaxFastImmutableArraySet<String> categories = getFastIntentCategories(intent);
...
//        sortResults(resultList);
    ...
    public List<R> queryIntent(Intent intent, String resolvedType, boolean defaultOnly,
            ...
            if (slashpos > 0) {
                ...
                if (!baseType.equals("*")) {
                    if (resolvedType.length() != slashpos+2
                        ...
                        if (debug) ParallaxSlog.v(TAG, "First type cut: " + Arrays.toString(firstTypeCut));
                        secondTypeCut = mWildTypeToFilter.get(baseType);
                        if (debug) ParallaxSlog.v(TAG, "Second type cut: "
                    ...
                    if (debug) ParallaxSlog.v(TAG, "Third type cut: " + Arrays.toString(thirdTypeCut));
                } else if (intent.getAction() != null) {
                    // The intent specified any type ({@literal *}/*).  This
                    // can be a whole heck of a lot of things, so as a first
                    // cut let's use the action instead.
                    firstTypeCut = mTypedActionToFilter.get(intent.getAction());
                    if (debug) ParallaxSlog.v(TAG, "Typed Action list: " + Arrays.toString(firstTypeCut));
                }
            }
        }
        ...
        if (scheme != null) {
            ...
            if (debug) ParallaxSlog.v(TAG, "Scheme list: " + Arrays.toString(schemeCut));
        }
        ...
        if (resolvedType == null && scheme == null && intent.getAction() != null) {
            ...
            if (debug) ParallaxSlog.v(TAG, "Action list: " + Arrays.toString(firstTypeCut));
        }
...
//        sortResults(finalList);
        ...
        if (debug) {
            ParallaxSlog.v(TAG, "Final result list:");
            for (int i=0; i<finalList.size(); i++) {
                ParallaxSlog.v(TAG, "  " + finalList.get(i));
            }
        }
        return finalList;
    }
    ...
    private final int register_mime_types(F filter, String prefix) {
        ...
        while (i.hasNext()) {
            String name = i.next();
            num++;
            if (localLOGV) ParallaxSlog.v(TAG, prefix + name);
    ...
    private static ParallaxFastImmutableArraySet<String> getFastIntentCategories(Intent intent) {
        final Set<String> categories = intent.getCategories();
        if (categories == null) {
            return null;
        }
        return new ParallaxFastImmutableArraySet<String>(categories.toArray(new String[categories.size()]));
    }

    private void buildResolveList(Intent intent, ParallaxFastImmutableArraySet<String> categories,
...
//        final boolean excludingStopped = intent.isExcludingStopped();
        ...
        for (i=0; i<N && (filter=src[i]) != null; i++) {
            int match;
            if (debug) ParallaxSlog.v(TAG, "Matching against filter " + filter);

//            if (excludingStopped && isFilterStopped(filter, userId)) {
//                if (debug) {
//                    ParallaxSlog.v(TAG, "  Filter's target is stopped; skipping");
//                }
//                continue;
//            }
            ...
            if (packageName != null && !isPackageForFilter(packageName, filter)) {
                if (debug) {
                    ParallaxSlog.v(TAG, "  Filter is not from package " + packageName + "; skipping");
                }
                continue;
            }
            ...
            if (!allowFilterResult(filter, dest)) {
                if (debug) {
                    ParallaxSlog.v(TAG, "  Filter's target already added");
                }
                continue;
            }
            ...
            if (match >= 0) {
                if (debug) ParallaxSlog.v(TAG, "  Filter matched!  match=0x" +
                ...
                if (!defaultOnly || filter.intentFilter.hasCategory(Intent.CATEGORY_DEFAULT)) {
                    ...
                    if (debug) ParallaxSlog.v(TAG, "    Created result: " + oneResult);
                    if (oneResult != null) {
                        dest.add(oneResult);
                    }
                } else {
                ...
                if (debug) {
                    ...
                    ParallaxSlog.v(TAG, "  Filter did not match: " + reason);
                }
            }
        }
        ...
        if (debug && hasNonDefaults) {
            if (dest.size() == 0) {
                ParallaxSlog.v(TAG, "resolveIntent failed: found match, but none with CATEGORY_DEFAULT");
            } else if (dest.size() > 1) {
                ParallaxSlog.v(TAG, "resolveIntent: multiple matches, only some with CATEGORY_DEFAULT");
            }
        }
    }
