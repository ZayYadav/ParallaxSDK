 ...
 */
public abstract class ParallaxBlackManager<Service extends IInterface> {
    public static final String TAG = "ParallaxBlackManager";
    ...
    public Service getService() {
        ...
        synchronized (mServiceLock) {
            ...
            try {
                IBinder binder = ParallaxCore.get().getService(getServiceName());
                if (binder == null) {
                    return null;
                }

                Service service = ParallaxReflector.on(getTClass().getName() + "$Stub")
