...
import com.Parallax.SDK.mirror.android.app.BRIActivityManagerL;
import com.Parallax.SDK.mirror.android.app.BRIActivityManagerN;
import com.Parallax.SDK.core.utils.compat.ParallaxBuildCompat;

public class ParallaxActivityManagerCompat {
	...
	public static boolean finishActivity(IBinder token, int code, Intent data) {
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
			return BRIActivityManagerN.get(BRActivityManagerNative.get().getDefault()).finishActivity(token, code, data, 0);
		} else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
			return BRIActivityManagerL.get(BRActivityManagerNative.get().getDefault()).finishActivity(token, code, data, false);
		}
		return false;
	}
