...
public class ParallaxLauncherActivity extends Activity {

    public static final String TAG = "ParallaxCoreLaunch";
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (launchIntent == null) {
            ParallaxSlog.e(TAG, "Launch intent is missing");
            finish();
            return;
        }
        ...
        if (packageName == null) {
            ParallaxSlog.e(TAG, "Package name is missing");
            finish();
            return;
        }

        PackageInfo packageInfo = ParallaxCore.getBPackageManager()
                .getPackageInfo(packageName, 0, userId);
        if (packageInfo == null) {
            ParallaxSlog.e(TAG, packageName + " is not installed");
            finish();
            return;
        }
    ...
    private void bindAppIdentity(PackageInfo packageInfo) {
        Drawable icon = packageInfo.applicationInfo
                .loadIcon(ParallaxCore.getPackageManager());
        CharSequence label = packageInfo.applicationInfo
                .loadLabel(ParallaxCore.getPackageManager());
    ...
    private void scheduleLaunch(Intent launchIntent, int userId) {
        ...
        mainHandler.postDelayed(() -> new Thread(() -> {
            try {
                ParallaxCore.getBActivityManager().startActivity(launchIntent, userId);
            } catch (Throwable throwable) {
                ParallaxSlog.e(TAG, "Unable to start isolated activity: " + throwable.getMessage());
                mainHandler.post(this::finish);
            }
        }, "ParallaxCoreLaunch").start(), 680L);
    }
