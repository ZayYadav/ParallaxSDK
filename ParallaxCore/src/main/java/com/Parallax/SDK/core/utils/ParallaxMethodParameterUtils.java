...
public class ParallaxMethodParameterUtils {

    public static <T> T getFirstParam(Object[] args, Class<T> tClass) {
        if (args == null) {
            return null;
        }
        int index = ParallaxArrayUtils.indexOfFirst(args, tClass);
        if (index != -1) {
            return (T) args[index];
        }
        return null;
    ...
    public static String replaceFirstAppPkg(Object[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            if (args[i] instanceof String) {
                ...
                if (ParallaxCore.get().isInstalled(value, ParallaxActivityThread.getUserId())) {
                    args[i] = ParallaxCore.getHostPkg();
                    return value;
                }
            }
        }
        ...
        for (int i = 0; i < args.length; i++) {
            if (args[i] instanceof Integer) {
                ...
                if (uid == ParallaxActivityThread.getBUid()) {
                    args[i] = ParallaxCore.getHostUid();
                }
            }
        }
    }
    ...
    public static void replaceLastUid(Object[] args) {
        int index = ParallaxArrayUtils.indexOfLast(args, Integer.class);
        if (index != -1) {
    ...
    public static String replaceLastAppPkg(Object[] args) {
        int index = ParallaxArrayUtils.indexOfLast(args, String.class);
        if (index != -1) {
            String pkg = (String) args[index];
            if (ParallaxCore.get().isInstalled(pkg, ParallaxActivityThread.getUserId())) {
                args[index] = ParallaxCore.getHostPkg();
            }
            return pkg;
        }
        return null;
    ...
    public static void replaceLastUserId(Object[] args) {
        ...
        if (index >= 0 && args[index] instanceof Integer) {
            args[index] = ParallaxCore.getHostUserId();
        }
    }
    
    public static String replaceSequenceAppPkg(Object[] args, int sequence) {
        int index = ParallaxArrayUtils.indexOf(args, String.class, sequence);
        if (index != -1) {
            String pkg = (String) args[index];
            if (ParallaxCore.get().isInstalled(pkg, ParallaxActivityThread.getUserId())) {
                args[index] = ParallaxCore.getHostPkg();
