...
This is a source/configuration security audit of the SDK, Loader, SDK panel, runtime-download path, and active release workflow. It is not a claim that a client running on a fully compromised/rooted device can be made impossible to modify. The goal is fail-closed authorization, strong server trust, minimized replay/tamper windows, and a hardened software supply chain.

## Executive summary

The V3 SDK authorization design is materially stronger than the surrounding build/runtime supply chain. V3 uses authenticated ECDH/AES-GCM transport, server ECDSA signatures, request/nonce binding, device-key proof, short leases, and redundant Java/native package/signing checks. The largest risks found were mutable executable-artifact delivery, mutable CI trust references that receive signing secrets, build-time auto-trust of the currently observed TLS certificate, and reverse-proxy header trust without an immediate-proxy allowlist.

The hardening branch fixes the high-impact supply-chain/configuration issues that can be changed without altering the SDK panel license/key-generation flow. Compatibility-sensitive changes (permission reduction and SDK R8/lint enforcement) are documented rather than blindly enabled.
...
1. Add a signed runtime-artifact manifest and verify SHA-256/size/signature before ZIP extraction and before `System.load`.
2. Add a V3 session validation/heartbeat endpoint that accepts the opaque session token, verifies its server-side hash, device/package/signing binding, revocation flag and expiry, and returns a short signed lease refresh.
3. Split the SDK manifest into a minimal default profile and an explicit virtualization/full-permission profile so consuming apps do not inherit unnecessary dangerous permissions.
4. Re-enable SDK release lint and R8 in stages with explicit reflection/JNI keep rules and regression tests rather than globally disabling checks.
5. Remove tracked signing fallback credentials; generate local debug/CI signing config outside source control.
6. Pin the remaining official GitHub Actions to immutable SHAs and let Dependabot update those pins by PR.
7. Add device-key attestation/hardware-backed policy as an optional high-assurance mode where supported; keep a documented fallback for devices without hardware attestation.
