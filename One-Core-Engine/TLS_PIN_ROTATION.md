...
OneCore Loader keeps OkHttp certificate pinning enabled for `teamdarkloader.teamdarkserver.online`.
...
`TeamDark_TLS_PINS` from GitHub Actions/Gradle is **additive**. Any valid pins supplied there are merged with the source-controlled pin and de-duplicated before `BuildConfig.TeamDark_TLS_PINS` is generated.
...
2. Add the new pin to the repository variable `TeamDark_TLS_PINS` while the old source-controlled pin is still valid.
