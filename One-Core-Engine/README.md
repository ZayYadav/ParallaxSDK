...
- Login accepts only activation keys generated by the OneCore Integrity
  dashboard. New deployments default to unique `5OC-…` keys, and owners may
  configure a 2-8 character branded prefix. The in-app key action opens that
  dashboard, avoiding incompatible legacy response schemas.
- CI validates that `ONECORE_API_ENCRYPTION_KEY` is valid base64 for exactly
  32 bytes and refuses to publish an APK with a missing/broken licensing key.
